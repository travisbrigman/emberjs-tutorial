...
module('Acceptance | shopping', function (hooks) {
  ...
  test('it should be able to add item in shopping cart ', async function (assert) {
    ...
    await click('[data-test-cart-link]');
    assert.equal(currentURL(), '/shopping-cart');
    assert
      .dom('[data-test-item-name]')
      .hasText('Beats Solo Wireless Headphones');
    assert.dom('[data-test-subtotal]').hasText('$99.98');
    assert.dom('[data-test-tax]').hasText('$8.99');
    assert.dom('[data-test-total]').hasText('$108.97');
  });
});
